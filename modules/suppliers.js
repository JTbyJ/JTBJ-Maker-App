          ...
          if (window.MAKER_CONFIG && window.MAKER_CONFIG.saveToDatabase) {
            for (const item of localData) {
              await window.MAKER_CONFIG.saveToDatabase('Suppliers', [
                item.id, item.name, item.category, item.status, item.rating,
                item.website, item.contact, item.email, item.phone,
                item.lead, item.minOrder, item.shipping, item.notes
              ]);
            }
          }
        } else {
          items = remoteDataParsed;
          render();
